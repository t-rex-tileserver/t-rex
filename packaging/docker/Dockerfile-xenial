...
#
# docker build --build-arg uid=$UID -t t-rex-xenial -f Dockerfile-xenial .
# docker run -t -i -v $(pwd):/var/data/in t-rex-xenial
# 
# Copy generated DEB package
# docker run --entrypoint="" t-rex-xenial ls -l /
# docker run --entrypoint="" -v $(pwd):/var/data/out t-rex-xenial cp /t-rex-0.8.0-dev-amd64-ubuntugis~xenial.deb /var/data/out/
...
>debian/DEBIAN/control && \
    ...
    mv debian.deb t-rex-$VERSION-$ARCH-ubuntugis~xenial.deb && \
    echo t-rex-$VERSION-$ARCH-ubuntugis-xenial.deb

RUN dpkg -i t-rex-*-ubuntugis~xenial.deb && rm -rf debian
